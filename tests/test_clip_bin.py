...
class TestClipBin(unittest.TestCase):
    def test_clip_bin_saver_and_picker_with_images(self):
    ...
    def test_clip_bin_saver_without_images_fallback(self):
        temp_dir = setup_temp_folder_paths()
        try:
    ...
    def test_index_auto_rebuild(self):
        temp_dir = setup_temp_folder_paths()
        try:
            v = torch.randn(1, 16, 4, 16, 16)
            latent = nodes.pack_av_latent(v, None)
    ...
    def test_auto_initial_and_chaining_unified_workflow(self):
        temp_dir = setup_temp_folder_paths()
        try:
            picker = nodes.MiniMaxClipBinPickerNode()
            saver = nodes.MiniMaxClipBinSaverNode()
    ...
    def test_safe_vae_decoders(self):
        class DummyVAE:
    ...
    def test_clip_bin_saver_video_file_name_from_vhs(self):
        temp_dir = setup_temp_folder_paths()
        try:
    ...
    def test_clip_bin_saver_auto_encode_video_from_images(self):
        temp_dir = setup_temp_folder_paths()
        try:
            v = torch.randn(1, 16, 4, 16, 16)
            latent = nodes.pack_av_latent(v, None)
    ...
    def test_delete_clip_asset(self):
        temp_dir = setup_temp_folder_paths()
        try:
            v = torch.randn(1, 16, 4, 16, 16)
            latent = nodes.pack_av_latent(v, None)
            ...
            # Delete
            deleted = delete_clip_asset("DeleteTest", clip_id)
            self.assertTrue(deleted)
            self.assertFalse(os.path.isdir(bin_dir))

            # Check index updated
            idx = load_project_index("DeleteTest")
            self.assertEqual(len(idx.get("clips", [])), 0)
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)
